    ...
    row.innerHTML = `
        ...
        <div class="multi-row-full">
            ...
            <div class="multi-photo-section" id="multiPhotoSection${n}" style="display:none;">
                <input type="file" accept="image/*" id="mImageInput${n}" style="display:none;" onchange="handleMultiImage(${n}, this)">
                <div class="multi-photo-preview" id="mImagePreview${n}" onclick="document.getElementById('mImageInput${n}').click()">
...
function removeMultiRow(n) { document.getElementById(`multiRow${n}`)?.remove(); }
...
function toggleMultiPhoto(n) {
    ...
    btn.innerHTML = open
        ? '<span class="material-icons" style="font-size:16px;">remove</span> Rimuovi foto'
        : '<span class="material-icons" style="font-size:16px;">add_a_photo</span> Aggiungi foto';
    btn.style.color = open ? '#e74c3c' : '';
    if (!open) {
...
if (btnOpenMultiModal) {
    btnOpenMultiModal.addEventListener('click', () => {
        ...
        multiRowCount = 0;
        createMultiRow(); createMultiRow();
        multiModal.classList.add('open');
    });
}
if (btnCloseMultiModal) btnCloseMultiModal.addEventListener('click', () => multiModal.classList.remove('open'));
if (btnAddRow) btnAddRow.addEventListener('click', createMultiRow);
...
if (btnSaveMulti) {
    btnSaveMulti.addEventListener('click', async () => {
        ...
        rows.forEach(row => {
            ...
            const image = document.getElementById(`mImagePreview${id}`)?.dataset?.image || '';
        ...
        for (let i = 0; i < shots.length; i++) {
            ...
            try {
                if (s.city) {
                    ...
                    if (geo && geo.length > 0) { s.lat = parseFloat(geo[0].lat); s.lon = parseFloat(geo[0].lon); }
